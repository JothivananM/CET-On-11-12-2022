...
function managerTimesheetConfirmation() {
    ...
    $.ajax({
        type: "GET",
        url:httpURL+"/directmanagertimesheetapproval?startDate="+$('#managerfromdate').val()+"&endDate="+$('#managertodate').val()+"&managerBusinessPartnerId="+internalID,
...
function managertableProjectFilter() {
    ...
    if (managerProjectName === "") {
        ...
        $('#managerstatus').find('option').remove().end().append('<option value="">All Status</option>');
        if(employeeName == ""){
            elementEmployee = managerTimesheetdata;
        }
        else{
            elementEmployee = managerTimesheetdata.filter(
            (task) => task.EmployeeName === employeeName
        );
        }
        ...
        if(employeeName == ""){
            ...
            element = managerTimesheetdata.filter(
                (task) => task.EmployeeName === employeeName
            );
            elementEmployee = element.filter(
                (task) => task.ProjectName === managerProjectName
...
function managertableTaskFilter() {
    ...
    if (managerProjectTask === "") {
        ...
        let employeeData;
        if(managerProjectName == ""){
            elementTask = managerTimesheetdata;
        }
        else if(employeeName =="" &&  managerProjectName != ""){
            elementTask = managerTimesheetdata.filter(
                (task) => task.ProjectName === managerProjectName
            );
        }
        else{
            employeeData =managerTimesheetdata.filter(
                (task) => task.EmployeeName === employeeName
            );
            elementTask = employeeData.filter(
                (task) => task.ProjectName === managerProjectName
            );
        ...
        $("#managerstatus").append('<option value="" selected>All Status</option>');
        if(managerProjectName == ""){
             taskelement = managerTimesheetdata.filter(
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#managerconfirmationtable tr").each(function () {
                    var ctrl = $(this).find("td:eq(0) input[type='checkbox']");
                    if (ctrl.prop("checked")) {
                        $(this).closest("tr")[0].cells[9].innerHTML = "Confirmed";
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#managerconfirmationtable tr").each(function () {
                        var ctrl = $(this).find("td:eq(0) input[type='checkbox']");
                        if (ctrl.prop("checked")) {
                            $(this).closest("tr")[0].cells[9].innerHTML = "Rejected";
...
function managerDateFilter() {
    ...
    if($("#managerfromdate").val() != "" && $("#managertodate").val() != ''){
        $("#exampleModalCenter").modal("show");
        ...
        toastr.error("From Date and To Date is Mandatory", { timeOut: 3000 });

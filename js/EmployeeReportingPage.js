...
function employeeReportingList() {
    ...
    $.ajax({
        type: "GET",
        url: httpURL+"/timesheetbookedmonthreport?BusinessPartnerId="+internalID+"&startDate="+$('#fromdate').val()+"&endDate="+$('#todate').val(),
...
function tableTaskFilter() {
    ...
    if (projectTask === "") {
        let elementProject;
        if(projectName == ""){
            elementProject = monthlyReportData;
        }
        else{
            elementProject = monthlyReportData.filter(
                (task) => task.ProjectName === projectName
            );
        }
        $('#school').find('option').remove().end().append('<option value="">All Schools</option>');
        ...
        activityList = [...new Map(elementProject.map(item => [item.ActivityName, item])).values()];
        activityList.forEach((element) =>{
            if(element.ActivityName !="" && element.ActivityName != undefined  ){
                $("#activity").append("<option>" + element.ActivityName + "</option>");
            }
        ...
        schoolList = [...new Map(elementProject.map(item => [item.SchoolDescription, item])).values()];
        schoolList.forEach((element) => {
            if(element.SchoolDescription !="" && element.SchoolDescription != undefined  ){
                $("#school").append("<option>" + element.SchoolDescription + "</option>");
            }
        });
        statusList = [...new Map(elementProject.map(item => [item.Status, item])).values()];
        statusList.forEach((element) => {
            if(element.Status !="" && element.Status != undefined  ){
                $("#status").append("<option>" + element.Status + "</option>");
            }
...
function dateFilter() {
    ...
    if($("#fromdate").val() != "" && $("#todate").val() != "" ){
        ...
        toastr.error("From Date and To date is Mandatory", { timeOut: 4000 });
    }

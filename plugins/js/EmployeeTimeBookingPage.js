...
// let projectListDetails = ['ARGENTINA', 
// 'AUSTRALIA', 
// 'BRAZIL', 
// 'BELARUS', 
// 'BHUTAN',
...
///Start Common Function
    ...
    function addSelect(divValue) {  
        ...
        $('#'+divValue+' .task').autocomplete({
            ...
            select: function( event, ui ) {
                ...
                $('.project').attr('data-projectid',taskFilter[0].ProjectId);
                $('.project').val(taskFilter[0].ProjectName);

                $('.activity').attr('data-activityid',taskFilter[0].ActivityID);
                $('.activity').val(taskFilter[0].ActivityName);  
            ...
            change: function (event, ui) {
                $('.task').val(taskFilter[0].TaskName);
                $('.task').attr('data-taskid',taskFilter[0].TaskID);
    ...
    //    Autocomplete Schools
        $( '#'+divValue+' .school'  ).autocomplete({
            ...
            change: function (event, ui) {
                ...
                $('.school').attr('data-schoolid',splittedText[0]);
                $('.school').val(splittedText[1]);
              },
            }).focus(function() {
            $(this).autocomplete("search", "");
          });
    ...
    function addCard(){
        ...
        // $("#cardTemplate").clone().attr('id', 'div'+cardCount).addClass('addedCard card-secondary').attr('style','display:block').insertAfter("#cardTemplate").fadeIn("slow");

        $("#cardTemplate").clone().attr('id', 'div'+cardCount).addClass('addedCard card-secondary').insertAfter("#cardTemplate").fadeIn("slow");
        
        $( '#div'+cardCount+' .cardHeader').text('New Task');
        $( '#div'+cardCount+' .btnClear').show();

        // Clear Card
        clearCard('#div'+cardCount,'new');


        addSelect('div'+cardCount);
        cardCount += 1; 
        if (cardCount >= 0) {
            $('#btn-addRemove').fadeIn();
        }
        // addSelect();
        // }
    }
...
/// Start On Calendar Event Click
        ...
        if (calendar.getEventById(cardDate+'N') || calendar.getEventById(cardDate+'C') || calendar.getEventById(cardDate+'R')) {
            ...
            if (filteredDetails.length != 0) {
                ...
                $.each(filteredDetails, function (index, value) { 
                    let statusValue = 'card-secondary';
                    
                    switch (value.Status) {
                        case 'Confirmed':
                            statusValue = 'card-success';
                            break;
        
                        case 'Rejected':
                            statusValue = 'card-danger';
                            break;
                    
                        default:
                            statusValue = 'card-secondary';
                            break;
                    }
        
                        if (index == 0) {
                                                        
                            $("#cardTemplate").clone().attr('id','div'+cardCount)
                            .addClass('addedCard '+statusValue)
                            
                            .attr('style','display:block').insertAfter('#cardTemplate');
                            ...
                            addSelect('div'+cardCount);
                                ...
                                $("#cardTemplate").clone().attr('id','div'+cardCount)
                                    ...
                                    $('#div'+cardCount).children('div').eq(0).attr('data-target','#collapse'+cardCount);
                                    $('#div'+cardCount).children('div').eq(1).attr('id','collapse'+cardCount);
                                    
                                    // $('#div'+cardCount+' div:first-child').data('target','collapse'+cardCount);
                                    // $('.dataTarget').data('target','collapse'+cardCount);
                                    // $('#div'+cardCount+' div:nth-child(2)').attr('id','collapse'+cardCount);
                                    // $('.dataId').attr('id','collapse'+cardCount);
    
                            }
                            if (value.Status == 'Not Confirmed' || value.Status == 'Rejected') {
                                if (value.Status == 'Rejected') {
                                    $('#div'+cardCount+' .div-rejectedReason').show();
                                    $('#div'+cardCount+' .rejectedReason').val(value.RejectionRemarks);
                                }
                                $('#div'+cardCount+' .btnSubmit').attr('value','Update');
                            }
                            $('#div'+cardCount+' .statusText').text(value.Status);
                            if (value.ProjectName == '') {
                                $( '#div'+cardCount+' .cardHeader').text(value.ActivityName);
                            }
                            else{
                            $( '#div'+cardCount+' .cardHeader').text(value.ProjectName);
                            }
                            $( '#div'+cardCount+' .cardDate').text("  ("+(value.Hours).substring(2,(value.Hours).length-1).replace('H',":")+")");
        
                            $( '#div'+cardCount+' .task').val(value.TaskName);
                            $( '#div'+cardCount+' .task').attr('data-taskid',value.TaskID);
                            $( '#div'+cardCount+' .task').attr('data-timeuuid',value.EmployeeTimeUUID);
                            $( '#div'+cardCount+' .task').attr('data-itemuuid',value.EmployeeItemUUID);
                            $( '#div'+cardCount+' .task').attr('data-timeagreementuuid',value.EmployeeTimeagreementUUID);
                            $( '#div'+cardCount+' .task').attr('data-timetextuuid',value.EmployeeTimeTextUUID);
    
                            employeeTimeagreementUUID = value.EmployeeTimeagreementUUID;
        
                            $( '#div'+cardCount+' .project').val(value.ProjectName);
                            $( '#div'+cardCount+' .project').attr('data-projectid',value.ProjectID);
        
                            $( '#div'+cardCount+' .activity').val(value.ActivityName);
                            $( '#div'+cardCount+' .activity').attr('data-activityid',value.ActivityID);
        
                            $( '#div'+cardCount+' .school').val(value.SchoolDescription);
                            $( '#div'+cardCount+' .school').attr('data-schoolid',value.SchoolID);
        
                            $( '#div'+cardCount+' .description').val(value.Description);
                            $( '#div'+cardCount+' .hours').val((value.Hours).substring(2,(value.Hours).length-1).replace('H',":"));
                            // $( '#div'+cardCount+' .cardDate').text("  ("+(value.Hours).substring(2,(value.Hours).length-1).replace('H',":")+")");
                            
                            $('#div'+cardCount+' .task').prop('disabled', true);
                            $('#div'+cardCount+' .activity').prop('disabled', true);
                            $('#div'+cardCount+' .school').prop('disabled', true);    
                        
                            if (value.Status == 'Confirmed') {
                                
                                $('#div'+cardCount+' .description').prop('disabled', true);
                                $('#div'+cardCount+' .hours').prop('disabled', true);
                                $('#div'+cardCount+' .btnClear').hide();
                                $('#div'+cardCount+' .btnSubmit').hide();
    ...
    function modifyTimesheet(event) {  
        ...
        notConfirmedHours = $('#'+cardId+' .hours').val();
        
        if ($('#'+cardId+' .activity').val() != '' && 
            $('#'+cardId+' .hours').val() !='' &&
            $('#'+cardId+' .description').val() != '') {
            ...
            if (hoursStatus == 'Update') {                           
                ...
                $("#exampleModalCenter").modal("show");
                    ...
                    .then(data => {
                        ...
                        $( '#'+cardId+' .cardHeader').text(data.TimesheetUpdate.ProjectName);
                            // $('#'+cardId+' .cardDate').text("  ("+data.TimesheetUpdate.Hours+")");
                            $('#'+cardId+' .cardDate').text("  ("+(data.TimesheetUpdate.Hours).substring(2,(data.TimesheetUpdate.Hours).length-1).replace('H',":")+")");
                            ...
                            $('#'+cardId+' .close-icon').CardWidget('toggle');
                ...
                $("#exampleModalCenter").modal("show");
                    ...
                    .then(response => {
                        toastr.success("Timesheet Booked Successfully.","Done!");
                            ...
                            $( '#'+cardId+' .cardHeader').text(response.TimesheetCreate.ProjectName);
                            $('#'+cardId+' .cardDate').text("  ("+(response.TimesheetCreate.Hours).substring(2,(response.TimesheetCreate.Hours).length-1).replace('H',":")+")");
